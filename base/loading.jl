...
function create_expr_cache(m::Expr)
    ...
    io, pobj = open(detach(setenv(`$(julia_cmd()) --build $(cachepath) -f --machinerepl`,["JULIA_HOME=$JULIA_HOME","HOME=$(homedir())"])), "w")
    serialize(io, quote
        empty!(LOAD_PATH)
        append!(LOAD_PATH, $LOAD_PATH)
        empty!(Base.LOAD_CACHE_PATH)
        append!(Base.LOAD_CACHE_PATH, $LOAD_CACHE_PATH)
        empty!(DL_LOAD_PATH)
        append!(DL_LOAD_PATH, $DL_LOAD_PATH)
    end)
